...
try:
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            pitch_scaled = int(pitch * 1000)
            yaw_scaled = int(yaw * 1000)

            # Create the UDP packet with the new data
            packet = (
                np.array([timestamp], dtype=np.int64).tobytes() +
                np.array([l_cx, l_cy, l_dx, l_dy, pitch_scaled, yaw_scaled], dtype=np.int32).tobytes()
            )
